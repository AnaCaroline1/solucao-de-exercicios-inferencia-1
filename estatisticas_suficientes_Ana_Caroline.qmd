...
subtitle: "Solução de exemplos e exerícios"
author: 
...
2.  E ${e^{-n\theta}\theta^{\sum_{i=1}^{n}x_i}}$ é a função $g_\theta$ que depende de $\theta$ e de $x_i$ somente através da *T*.

Nota-se que $T(x_1,x_2,...,x_n)=\sum_{i =x_i}^{n}$. Logo, $\sum_{i =x_i}^{n}$ é uma estatística suficiente para $\theta$.
